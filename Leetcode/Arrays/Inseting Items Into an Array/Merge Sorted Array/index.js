...
class MergeSortedArray {
  ...
  Merge() {
    /*
        loop trough first array lenth - repalcable and secound arra (m+n)
        start from the end of m or where you want to start inserting at
        then it just current index - m 
    */
    for (let i = this.m; i < this.m + this.n; i++) {
      this.nums1[i] = this.nums2[i - this.m];
    }

    return this.nums1.sort((a, b) => a - b);
  }
}

console.log(new MergeSortedArray([1, 2, 3, 0, 0, 0], 3, [2, 5, 6], 3).Merge());
